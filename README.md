# ABC FC Foundation Website

Official website for **ABC FC Foundation (NPC)** — African by Choice Football Club.

## About

ABC FC Foundation is a registered Non-Profit Company (NPC) founded by **Lucas Tshibalo Mudau, MPA**. The foundation uses the power of football to inspire and support less privileged communities across South Africa, primarily based in Makwarela, Thohoyandou, Limpopo (Vhembe District).

The club (African by Choice FC, est. 2008) competes in the **Hollywoodbets Regional League** under **SAFA Vhembe Region**, where it was crowned **Stream I Champions 2025/26**.

## Club Identity

- **Full Name:** African by Choice Football Club (ABC FC)
- **Nickname:** Lion of the North
- **Founded:** 2008
- **Home Ground:** Phalama Ground, Mutale
- **Location:** Makwarela, Thohoyandou, Limpopo, South Africa
- **League:** Hollywoodbets Regional League — SAFA Vhembe Region (Stream I Champions 2025/26)
- **Colours:** Amber Gold (#F5A800), Black (#111111), White (#FFFFFF)
- **Social:** [Facebook](https://www.facebook.com/p/ABC-fc-61556965480952/)

## Website Sections

1. **Hero** — Full-screen hero with team identity
2. **About** — Club history, values (Attitude, Believe, Courage), founder info
3. **Foundation** — Mission, community pillars, outreach timeline (2018–2024)
4. **Programs** — SAB League, ABC Grow Learning, Social Outreach, Player Trials
5. **The Club** — Location, league, legal status, colours, founder profile
6. **Sponsorship** — Partnership opportunities
7. **Contact** — Location, phone, email, social links, contact form

## Tech Stack

- Pure HTML5, CSS3, JavaScript (no frameworks/dependencies)
- Google Fonts: Oswald (display) + Inter (body)
- Fully responsive — mobile-first design
- Dark/Light mode toggle
- Scroll reveal animations
- Semantic HTML & accessible

## Founder

**Lucas Tshibalo Mudau, MPA**
- Chairman & Founder — ABC FC Foundation (NPC)
- Chairman & Owner — African by Choice Football Club
- Group Head, Group SHELA & FCM (GCSS) — City of Johannesburg
- Master of Public Administration — University of Pretoria (2011–2012)

## Contact

- Email: tshibalo.lucas@gmail.com
- Phone: +27 71 109 2360
- Location: Thohoyandou, Vhembe District, Limpopo, South Africa

---

© 2024 ABC FC Foundation (NPC). All rights reserved.
